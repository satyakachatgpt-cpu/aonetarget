...
interface Subject { id: string; name: string; course: string; icon: string; status: 'active' | 'inactive'; createdDate: string; }

interface Props { showToast: (m: string, type?: 'success' | 'error') => void; hideHeader?: boolean; }

const Subjects: React.FC<Props> = ({ showToast, hideHeader }) => {
  ...
  const [formData, setFormData] = useState({ name: '', course: '', icon: '', status: 'active' as 'active' | 'inactive' });

  useEffect(() => { loadItems(); }, []);

  const loadItems = async () => {
    try {
      const data = await subjectsAPI.getAll().catch(() => []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = {
        id: editingItem?.id || `subject_${Date.now()}`,
        name: formData.name,
      ...
      if (editingItem) {
        await subjectsAPI.update(editingItem.id, data);
        showToast('Updated!');
      } else {
      ...
      setShowModal(false);
      setEditingItem(null);
      setFormData({ name: '', course: '', icon: '', status: 'active' });
      loadItems();
    } catch (error) {
    ...
    <div className="space-y-6">
      {!hideHeader && (
        <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div><h3 className="text-lg font-black text-navy uppercase tracking-widest">Subjects</h3><p className="text-[10px] font-bold text-gray-400 mt-1 uppercase">Total: {items.length}</p></div>
          <button onClick={() => { setEditingItem(null); setFormData({ name: '', course: '', icon: '', status: 'active' }); setShowModal(true); }} className="bg-navy text-white px-6 py-3 rounded-xl font-black text-[11px] uppercase shadow-sm hover:shadow-md hover:scale-105 transition-all flex items-center gap-2"><span className="material-icons-outlined text-base">add</span> Add</button>
        </div>
      )}
          ...
          {isFilterOpen && (
            <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl z-[100] p-4 animate-in fade-in zoom-in duration-200 origin-top-right">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-2">Subject Status</label>
                  <div className="flex flex-col gap-1">
                    {['all', 'active', 'inactive'].map((status) => (
                      ...
                      >
                        {status === 'all' ? 'All Subjects' : status === 'active' ? 'Active Only' : 'Inactive Only'}
                      </button>
                    ))}
                  </div>
                </div>
