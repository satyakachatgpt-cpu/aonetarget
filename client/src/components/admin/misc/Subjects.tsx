...
const Subjects: React.FC<Props> = ({ showToast, hideHeader }) => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      await subjectsAPI.reorder(orderedIds);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {paginatedItems.length === 0 ? (
          <div className="p-12 text-center">
            <span className="material-icons-outlined text-5xl text-gray-200 block mb-4">subject</span>
            <p className="text-gray-400 font-bold uppercase">No subjects found</p>
          </div>
        ) : (
          <>
                    ...
                    >
                      {paginatedItems.map((item, idx) => (
                        <SortableRow
                          ...
                          onEdit={(item: Subject) => {
                            setEditingItem(item);
                            setFormData({ name: item.name, course: item.course, icon: item.icon, status: item.status });
                            setShowModal(true);
                          }}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-lg w-full max-w-2xl border border-gray-100">
            <div className="flex justify-between items-center px-8 py-6 border-b border-gray-100">
              <h3 className="text-lg font-black text-navy uppercase tracking-widest">{editingItem ? 'Edit Subject' : 'Add Subject'}</h3>
            ...
            <div className="p-8 space-y-4">
              <div>
                ...
                <input type="text" placeholder="Enter name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-lg font-bold outline-none focus:ring-2 focus:ring-navy/10" />
              </div>
              <div>
                <label className="block text-xs font-black text-gray-700 uppercase mb-2">Course</label>
                <input type="text" placeholder="Enter course" value={formData.course} onChange={(e) => setFormData({ ...formData, course: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-lg font-bold outline-none focus:ring-2 focus:ring-navy/10" />
              </div>
              <div>
