    ...
    <div className="flex items-center gap-3">
      ...
      <div className="relative">
        <button
          ...
          className={`w-[44px] h-[44px] flex items-center justify-center rounded-[10px] border transition-all ${
            isFilterOpen 
          ...
          <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-100 rounded-xl shadow-xl p-2 z-[100] animate-in fade-in zoom-in duration-200 origin-top-right">
            ...
            {filterOptions.map((opt) => (
              <button
                ...
                className={`w-full text-left px-3 py-2 text-[13px] font-bold rounded-lg transition-colors ${
                  contentTypeFilter === opt.value ? 'bg-black text-white' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
                {opt.label}
              </button>
