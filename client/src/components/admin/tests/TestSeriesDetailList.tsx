...
const SortableTestItem = ({ 
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`bg-white border border-gray-100 shadow-sm hover:shadow-md transition-all ${isDragging ? 'shadow-2xl' : ''}`}
    >
      <div className="px-5 py-4 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-1 text-[12px] text-gray-500 font-medium">
            ...
            {test.sortBy && (
              <span className="text-gray-400 text-[11px] ml-1">Pos: {index + 1}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-auto">
          ...
          <div className="relative action-menu-container" onMouseDown={(e) => e.stopPropagation()} onTouchStart={(e) => e.stopPropagation()}>
            <button
              onClick={() =>
                setActiveActionMenuId(
                  activeActionMenuId === (test.id || (test as any)._id)
                    ? null
      ...
      <div className="grid grid-cols-1 gap-4">
        {tests.length === 0 ? (
          <div className="bg-white border border-dashed border-gray-200 rounded-xl p-12 text-center">
            ...
            <p className="text-gray-500 font-medium">No tests found in this series</p>
          </div>
        ) : (
          <SortableContext
            items={tests.filter(t => !!(t._id || t.id)).map(t => String(t._id || t.id))}
            strategy={verticalListSortingStrategy}
            disabled={isSortingDisabled}
          >
            {tests.map((test, index) => (
              <SortableTestItem
                key={String(test._id || test.id || index)}
                test={test}
                index={index}
                activeActionMenuId={activeActionMenuId}
                setActiveActionMenuId={setActiveActionMenuId}
                onViewQuestionEditor={onViewQuestionEditor}
